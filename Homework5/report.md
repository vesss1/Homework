...
圖可以分為有向與無向兩種，因此在設計時需要同時考慮這兩種情況。  

在資料結構的選擇上使用相鄰串列（Adjacency List）來儲存每個頂點的鄰接關係，這樣在進行 Depth First Search 或 Breadth First Search 這種圖的遍歷時，可以有效找到相鄰節點，另外維護一份邊的集合，方便在需要對所有邊進行處理時可以直接使用。  

演算法部分本題涵蓋了幾個圖論中的核心應用，包括圖的遍歷、生成樹以及最短路徑等，這些方法可以用來判斷圖的連通性、建立最小成本的連線方式，或找出兩點之間的最佳路徑。  

在程式操作上透過指令方式讓使用者可以選擇不同功能來測試同一張圖，例如執行遍歷、計算路徑或分析結構等。這樣的設計可以避免每次修改功能都需要重新編譯程式，也讓整體測試流程更加方便。
