...
**解題說明**
本題主要是建立一個 Graph 類別，並透過不同的演算法來分析圖的結構與性質。圖可以分為有向與無向兩種，因此在設計時需要同時考慮這兩種情況。

在資料結構的選擇上，採用相鄰串列（Adjacency List）來儲存每個頂點的鄰接關係，這樣在進行像 Depth First Search 或 Breadth First Search 這類圖的遍歷操作時，可以有效率地找到相鄰節點。同時，另外維護一份邊的集合，方便在需要對所有邊進行處理時（例如生成樹相關演算法）可以直接使用。

在演算法部分，本題涵蓋了幾個圖論中的核心應用，包括圖的遍歷、生成樹以及最短路徑等。這些方法可以用來判斷圖的連通性、建立最小成本的連線方式，或找出兩點之間的最佳路徑。

另外，在程式操作上，透過指令方式讓使用者可以選擇不同功能來測試同一張圖，例如執行遍歷、計算路徑或分析結構等。這樣的設計可以避免每次修改功能都需要重新編譯程式，也讓整體測試流程更加方便。

整體來說，本題的重點在於結合適當的資料結構與演算法，讓圖的各種操作可以有效率地完成，同時也讓不同功能之間可以相互搭配使用。
---
