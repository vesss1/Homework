...
# 作業5  **Graph**
## **解題說明**
這題主要是在做圖（Graph）的整合程式，把幾個常見演算法全部寫在同一支程式裡面。  
...
這次在實作 Graph 時，我們使用 **鄰接矩陣（Adjacency Matrix）** 來存圖。雖然鄰接串列在空間上較省，但因為本題需要做 Floyd、Reach 等演算法，矩陣在查詢上比較直覺，也比較好實作。

程式一開始會輸入節點數、邊數以及圖的種類（有向或無向）。若是無向圖，會同時設定 `g[a][b]` 和 `g[b][a]`；有向圖則只設定單方向。

除了矩陣外，也另外使用 `Edge` 陣列來存所有邊，方便用在 Kruskal 和 Bellman-Ford 這類需要逐邊處理的演算法。

走訪部分實作了 DFS 和 BFS，並用 `vis[]` 避免重複拜訪。Connected Components 則是透過多次 DFS 找出無向圖的連通區塊。

在生成樹方面，實作了 Kruskal 和 Prim。Kruskal 透過排序邊並搭配 Union-Find，Prim 則是從起點逐步擴展。

最短路徑包含 Dijkstra、Bellman-Ford 和 Floyd，分別處理不同情況。Reach 用來判斷節點之間是否可達。

有向圖部分則實作 AOV（拓撲排序）與 AOE（關鍵路徑），用來分析節點順序與最長路徑。

另外也用 DFS 搭配 `dfn` 和 `low` 找出雙連通分量。為避免數值問題，權重使用 `long long`，並用 `INF` 表示不可到達。

整體透過選單控制不同功能，讓同一張圖可以測試多種演算法，結構也比較清楚。

---

## 結論

這次實作讓我們把多個圖論演算法整合在同一個程式中，對整體架構有更清楚的理解。

過程中可以發現，資料結構的選擇會影響實作難度。使用鄰接矩陣讓部分演算法更容易處理，而搭配 Edge List 也讓程式更有彈性。

另外，不同演算法的適用情境也變得更明確，例如最短路徑的方法選擇，以及拓撲排序對 DAG 的限制。

整體來說，這題幫助我們把觀念和實作連結起來，對圖的理解也更完整。
