...
| 測試項目 | 測試輸入 / 操作 | 預期結果 | 驗證結果 |
|---|---|---|---|
| DFS | 選擇 `1`，輸入起點 `0` | 從節點 0 開始進行深度優先搜尋 | 可輸出 DFS 走訪順序 |
| BFS | 選擇 `2`，輸入起點 `0` | 從節點 0 開始進行廣度優先搜尋 | 可輸出 BFS 走訪順序 |
| Connected Components | 選擇 `3` | 找出無向圖中的所有連通分量 | 可列出各個 Component |
| Kruskal | 選擇 `4` | 找出最小生成樹的邊與總權重 | 可輸出 MST 邊與 Total |
| Dijkstra | 選擇 `5`，輸入起點 `0` | 計算起點到其他節點的最短距離 | 可輸出各節點最短距離 |
| Floyd-Warshall | 選擇 `6` | 計算所有點對最短路徑 | 可輸出距離矩陣 |
| Transitive Closure | 選擇 `7` | 判斷任兩點之間是否可達 | 可輸出 0 / 1 可達矩陣 |
| Adjacency Matrix | 選擇 `8` | 顯示目前圖的鄰接矩陣 | 可正確印出 Matrix |
| Prim | 選擇 `9`，輸入起點 `0` | 找出最小生成樹 | 可輸出 Prim MST 結果 |
| Spanning Tree | 選擇 `10`，輸入起點 `0` | 建立一棵生成樹 | 可輸出生成樹邊 |
| Biconnected Components | 選擇 `11` | 找出雙連通分量 | 可輸出 Biconnected component |
| Bellman-Ford | 選擇 `12`，輸入起點 `0` | 計算含一般權重的最短路徑 | 可輸出最短距離或負環提示 |
| AOV | 選擇 `13` | 對有向無環圖進行拓撲排序 | 可輸出拓撲順序 |
| AOE | 選擇 `14` | 計算活動網路的關鍵路徑 | 可輸出 ve、vl 與 Critical edges |
| Exit | 選擇 `0` | 結束程式 | 程式正常結束 |
