...
| 測試編號 | 測試內容 | 輸入示意式A | 輸入示意式B | 預期結果（以 A + B 為例） | 實際產出（填入程式輸出） |
|---|---|---|---|---|---|
| T1 | 基本加法運算 | \(2x^2 + 3x + 4\) | \(5x + (-4)\) | \(2x^2 + 8x\) |  |
| T2 | 不同次方數量 | \(5x^3 + 3x + 2\) | \(2x^2 + 7\) | \(5x^3 + 2x^2 + 3x + 9\) |  |
| T3 | 同次方合併結果為 0 | \(3x^2 + 4\) | \(-3x^2 + 5\) | \(9\) |  |
| T4 | 輸入為 0 計算式 | \(0\) | \(3x^1 + 1\) | \(3x^1 + 1\) |  |
| T5 | 大量項目（效能測試） | A、B 各含 1000 項 | A、B 各含 1000 項 | 執行時間 < 0.01 秒（視環境） |  |
